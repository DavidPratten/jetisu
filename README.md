...
| Derived table         | Query in Relational Algebra, SQL, Datalog, including fixed point operators| Generated on demand or cached for reuse.
...
__Do not repeat yourself:__ The relational algebra is inherently agnostic to which attributes are known and used to constrain the relation and which attributes are unknown and which are sought as the answer. An intensionally defined relation may be used to query rules "forwards" or in "reverse" depending on what is known prior to querying.
